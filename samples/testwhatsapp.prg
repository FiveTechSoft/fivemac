...
function Main()
   ...
   local oWnd
   local cTo, cMessage 
   local aUsers:= { { "34684078311","359028035490117", "lukilla" },{ "34123456789","4444444444444","mastintin" } }
   ...
   @ 200, 100 COMBOBOX oCbx VAR cUser OF oWnd ;
      SIZE 124, 25 ITEMS aUserName    
   
   @ 150, 40 SAY "Phone:" OF oWnd
   @ 150, 100 GET oGet VAR cPhone OF oWnd TOOLTIP "Intro Phone "
   oGet:setnumeric()

   
   @ 110, 40 SAY "Message:" OF oWnd
   @ 68, 100 GET oGet2 VAR cText OF oWnd TOOLTIP "Intro Message " SIZE 250,60
   
   
       
   @ 10, 250 BUTTON "Send" OF oWnd ACTION  MainWhats(aUsers[oCbx:nItemSelected()],alltrim(cPhone),alltrim(cText))
...
function MainWhats(aUserData,cPhone,cText)
local cNumber:= aUserData[1]
local cIMEI:=   aUserData[2]
local cNickName := aUserData[3]


local oWA := HB_WhatsApp():New(cNumber,hb_md5(StrRev(cIMEI)),cNickname)
  
   // msginfo(hb_md5(StrRev(cIMEI)) )

   oWA:Connect()
   oWA:Login()
     // ? oWA:cAccount_status, oWA:cAccount_kind, oWA:cAccount_creation, oWA:cAccount_expiration
     
   if oWA:cAccount_status != "active"
     msginfo("no activo")
     return nil
   endif
   oWA:Message( Str( TS2Num() ), cPhone , cText )
